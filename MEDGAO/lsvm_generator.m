...
if (~useold)
    ...
    save(['/home/liujiang/',test_object,'_GAO_train_set.mat'],'train_set','-v7.3');   %save the test_set, for the convenience of test_engine.
    cd(root);

    %save test set to file, for lsvm_test_engine use.
    save(['/home/liujiang/',test_object,'_GAO_test_set.mat'],'test_set','-v7.3');   %save the test_set, for the convenience of test_engine.
    clear test_set;  
else
    load(['/home/liujiang/',test_object,'_GAO_train_set.mat']);
end
...
for itecount=1:100   %begin iteration. maximum iteration count is 100.
   if (exist(['lsvm_model_gao/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_',num2str(itecount),'.mat'],'file'))
       load(['lsvm_model_gao/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_',num2str(itecount),'.mat']);
       disp('Model existed..');
   else
  ...
  if (itecount==1)
        save(['lsvm_model_gao/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_0.mat'],'models');  %save default mean-pooling strategy for comparable test.
  end
  ...
  for object_bank_word=1:total_word
   ...
   save(['lsvm_model_gao/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_',num2str(itecount),'.mat'],'models');
  ...
  for i=1:size(train_set_global,2)                      %train_set_global{1}: pos examples. train_set_global{2}: nega examples.
    for j=1:size(train_set_global{i}.trains,2)        
        ...
        if (i==1 || i==2)
            ...
            feat_new=feat_new(:,(object_bank_word-1)*1000+1:(object_bank_word)*1000);
            % feat_new=feat_new(:,1:(object_bank_word)*1000);
            if(isempty(feat_new)), continue; end;    % a single video's final pooling result after multiplied by theta.
     
            vec=theta*feat_new;
            
           % vec=vec((object_bank_word-1)*1000+1,object_bank_word*1000);
...
for i=1:size(train_set,2)
  ...
  %  for j=1:floor(size(train_set{i}.trains,2)*train_w_b_ratio)   %specify w b's data.
     for j=1:size(train_set{i}.trains,2)   %cancel the w_b validation strategy.
      ...
      %  disp(['size of feat_new=',num2str(size(feat_new,1)),'*',num2str(size(feat_new,2))]);
       feat_new=double(feat_new(:,(object_bank_word-1)*1000+1:(object_bank_word)*1000));
     %   feat_new=feat_new(:,1:(object_bank_word)*1000);
        if(isempty(feat_new))
            continue;
        end;
        ...
        %vec=vec((object_bank_word-1)*1000+1,(object_bank_word)*1000);  %optimize each word.
        if (i==1)
            train_label=[train_label;1];   %belongs to present 1-n svm's positive examples.
            train_feat=[train_feat;vec];
            %spos=spos+1;
