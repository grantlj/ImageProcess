...
%box的物理意义：每一行是检测到的一个东西，最后一列，表示该东西的得分，倒数第二列表示，用到的component；
%前4列表示root filter的bonding box；
%后面表示每个part的bounding box;

if nargin > 6 && ~isempty(bbox)
  latent = true;                         
else
  latent = false;               %latent is false;
end

if nargin > 9 && fid ~= 0
  write = true;
else
  write = false;               %write is false;
end
...
for i = 1:length(model.rootfilters)
  rootfilters{i} = model.rootfilters{i}.w;       %model的rootfilter的w就是DPM里面训练出来的主检测器的weight
end

partfilters = [];
for i = 1:length(model.partfilters)
  partfilters{i} = model.partfilters{i}.w;      %model的partfilter的w就是DPM里面训练出来的辅助检测器的weight
end
responsemap = {};

% cache some data  
for c = 1:model.numcomponents                    %这里应该是子检测器的个数？
...
% the feature pyramid
interval=model.interval;                                             %model的interval？
...
for level = Level                                               %level表示的是尺度大小，resolution；
  ...
  rootmatch = fconv(featr, rootfilters, 1, length(rootfilters));             %哦，这个是最主要的，做卷积，算分数了！rootmatch表示检测模型
                                                                             %本身的匹配度；
  ...
  for c = 1:model.numcomponents
    % root score + offset
    score = rootmatch{ridx{c}} + model.offsets{oidx{c}}.w;                 %得分=root分数=root的相似度+offset的weight；
    ...
    for j = 1:numparts{c}
      ...
      score = score - M(ay{c,j}:2:ay{c,j}+2*(size(score,1)-1), ...                          %DPM模型，弹簧形变扣分；
                        ax{c,j}:2:ax{c,j}+2*(size(score,2)-1));                             %score貌似是整个图片在某个part filter上的
                                                                                            %response！！
    end
    responsemap{end+1} = score;                                                              %responsemap添加一个score，最后一行，最后一列；
                                                                                             %score在这里是一个mat哦！
    if ~latent
      % get all good matches
      I = find(score > thresh);
      [Y, X] = ind2sub(size(score), I);                                                      %这是什么？返回subscript？找分数最大的位置。
      tmp = [];
      for i = 1:length(I)
        x = X(i);
        y = Y(i);
          ...
          if score(y, x) > best
            ...
            if w > 0 && h > 0
              ...
              if (o >= overlap)
                ...
                boxes = [boxes c best];                     %这些操作好像是为了得到最好的box结果？（默认只有一个？）
              end
            end
          end
        end
...
function [x1, y1, x2, y2] = rootbox(x, y, scale, padx, pady, rsize)            %这是啥？画bounding box的？
x1 = (x-padx)*scale+1;
y1 = (y-pady)*scale+1;
