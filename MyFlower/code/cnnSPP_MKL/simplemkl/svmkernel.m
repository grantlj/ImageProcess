...
case 'gaussian'
    ...
    if nk ~=nk2
        ...
        kerneloption=ones(1,n2)*kerneloption;  %将kerneloption应用到所有维度
    end;
    ...
    metric = diag(1./kerneloption.^2); %构造对角线矩阵
    ps = x*metric*xsup';  %构造一个内积，感觉这里的kerneloption好像是一个scaler相关的参数!!!如果option是1，那么就是直接内积 这个算的是X1*X2
    [nps,pps]=size(ps);
    normx = sum(x.^2*metric,2); %这个是算单个vector的norm 他这个就是算X1^2 X2^2
    normxsup = sum(xsup.^2*metric,2);%算支撑向量的norm
    ps = -2*ps + repmat(normx,1,pps) + repmat(normxsup',nps,1) ; %算出来距离了！！！！
