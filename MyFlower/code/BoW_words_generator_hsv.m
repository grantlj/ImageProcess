function [] = BoW_words_generator_hsv()
 raw_feat_path='../dataset/raw_feat/';
 data_splits_path='../dataset/datasplits.mat';
 output_file_path='../dataset/hsv_words_centers.mat';
 load(data_splits_path);
 
 img_count=1360;
 img_sample_count=300;
 
 feat_mat=rand(img_sample_count*img_count,3);
  ...
  for i=1:size(trn1,2)
     ...
     for j=1:img_sample_count
       total=total+1;
       feat_mat(total,:)=feat(sample_index_x(j),sample_index_y(j));
     end
  end
  
  disp('Doing Clusting...');
  [centers, assignments]=vl_kmeans(feat_mat', 200);
