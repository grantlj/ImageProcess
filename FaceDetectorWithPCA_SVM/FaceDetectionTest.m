function []=FaceDetectionTest()
   ...
   faceThreshholdDir='D:\Matlab\MitFace\Threshhold.dat'; %存储脸部到meanFace的最远距离
   threshhold=load(faceThreshholdDir);
  ...
  for i=1:(160-window_h)/step_row+1;
    ...
    % imshow(l_selected);
          ...
          tmp=double(sum(l_selected_info.^2));
          
          if (tmp<=threshhold*0.7)
