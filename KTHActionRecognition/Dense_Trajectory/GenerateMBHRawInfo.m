...
%返回某一视频文件在所有spatial下的trajectory描述子结构体
  ...
  disp('*****Initialize Video...');
   ...
   for i=1:spatial_size
     ...
     ofInfo=GetOpticalFlowInfo(mov);                           %获取当前维度光流信息;
     ret=[ret;GetMBH(mov,ofInfo)];                             %根据当前维度光流信息，获取MBH。
   end
...
function [traMat]=GetMBH(mov,ofInfo)
  ...
  pt_mbh=cell(size(pt_x,2),size(pt_y,2),L);                %记录每个点的15帧对应的MBH信息  行号，列号，帧数，MBHx，MBHy堆叠存放       
...
%           防止越界
          ...
          srInfo.l=floor(max(1,cood(i,j,p,2)-15));srInfo.r=floor(min(width,cood(i,j,p,2)+16));    %确认region的左右位置;
          srInfo.t=floor(max(1,cood(i,j,p,1)-15));srInfo.b=floor(min(height,cood(i,j,p,1)+16));   %确认region的上下位置;
          
          if ((srInfo.r-srInfo.l==31) && (srInfo.b-srInfo.t==31))          %确认block大小固定为32*32
              region=u(srInfo.t:srInfo.b,srInfo.l:srInfo.r);               
              t1=GetBlockMBH(region);                           %计算MBHx
              region=v(srInfo.t:srInfo.b,srInfo.l:srInfo.r);
              t2=GetBlockMBH(region);                           %计算MBHy,然后和MBHx拼在一起
              x_y=[t1,t2];
              pt_mbh{i,j,p}=x_y;
          else 
