 ...
 content_feat_root_path=[dataset_root_path,'feat_content_spp_vgg_16_l31_finetuned\']; mkdir(content_feat_root_path);
 context_feat_root_path=[dataset_root_path,'feat_context_spp_vgg_16_l31_finetuned\']; mkdir(context_feat_root_path);
...
% raw_trn_feat_mat=zeros(trn_count,4096); raw_tst_feat_mat=zeros(tst_count,4096);
 ...
 for i=1:trn_count
    content_feat_path=[content_feat_root_path,'\image_',sprintf('%05d',trnid(i)),'.mat'];
    context_feat_path=[context_feat_root_path,'\image_',sprintf('%05d',trnid(i)),'.mat'];
    %raw_feat_path=[raw_feat_root_path,'\image_',sprintf('%05d',trnid(i)),'.mat'];
 ...
 %Loading test feature.
  for i=1:tst_count
    content_feat_path=[content_feat_root_path,'\image_',sprintf('%05d',tstid(i)),'.mat'];
    context_feat_path=[context_feat_root_path,'\image_',sprintf('%05d',tstid(i)),'.mat'];
    %raw_feat_path=[raw_feat_root_path,'\image_',sprintf('%05d',tstid(i)),'.mat'];
...
%final_prob=prob_1+prob_2;
get_accuracy_by_prob(final_prob,tst_label,102)
