...
content_db_root_path='D:\dataset\oxfordflower102\feat_content_spp_vgg_16_l31_finetuned\';
context_db_root_path='D:\dataset\oxfordflower102\feat_context_spp_vgg_16_l31_finetuned\';
...
net.layers{1,1}= struct('type', 'conv', ...
                           'filters', 0.01/scal * randn(1,10752,1,64,'single'),...
...
net.layers{1,3}=struct('type','conv',...
                       'filters', 0.01/scal * randn(1,10752*2,1,64,'single'),...
                       'biases', init_bias*ones(1,64,'single'), ...
                       'stride', 1, ...
                       'pad', 0, ...
                       'filtersLearningRate', 1, ...
...
net.layers{2,1}= struct('type', 'conv', ...
                           'filters', 0.01/scal * randn(1,1,64,102,'single'),...
                           'biases', init_bias*ones(1,102,'single'), ...
                           'stride', 1, ...
                           'pad', 0, ...
                           'filtersLearningRate', 1, ...
                           'biasesLearningRate', 2, ...
...
net.layers{2,3}= struct('type', 'conv', ...
                           'filters', 0.01/scal * randn(1,1,64,2,'single'),...
...
function [content_feats, context_feats, img_labels]=load_features(sel_imgs,content_db_root_path,context_db_root_path,labels)
  img_labels=labels(sel_imgs);
  content_feats=[];context_feats=[];
  for i=1:size(sel_imgs,2)
    ...
    content_feats=[content_feats;content_feat];
    
    %load context feat.
    load(now_img_context_path);
    context_feats=[context_feats;context_feat];
  end
