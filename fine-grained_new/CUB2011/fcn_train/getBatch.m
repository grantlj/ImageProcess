...
opts.labelStride = 1 ;
opts.labelOffset = 0 ;
opts.classWeights = ones(1,201,'single') ;
...
for i=1:numel(images)
  ...
  % acquire image
  if isempty(im{i})
    rgbPath = imdb.images.im_pathlist{images(i)} ;
    labelsPath = imdb.images.seg_pathlist{images(i)} ;
    rgb = vl_imreadjpeg({rgbPath}) ;
    rgb = rgb{1} ;
    
    now_label=imdb.images.labels(images(i));
    anno = imread(labelsPath) ;
    
    anno(anno~=255)=1;
    anno(anno==255)=now_label+1;  %label =1 is background.
