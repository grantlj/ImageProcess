...
filters = single(bilinear_u(4, 1, 201)) ;
...
net.addLayer('skip4', ...
     dagnn.Conv('size', [1 1 512 201], 'pad', 0), ...
     'x24', 'x40', {'skip4f','skip4b'});

f = net.getParamIndex('skip4f') ;
net.params(f).value = zeros(1, 1, 512, 201, 'single') ;
net.params(f).learningRate = 0.1 ;
net.params(f).weightDecay = 1 ;

f = net.getParamIndex('skip4b') ;
net.params(f).value = zeros(1, 1, 201, 'single') ;
...
filters = single(bilinear_u(32, 201, 201)) ;
