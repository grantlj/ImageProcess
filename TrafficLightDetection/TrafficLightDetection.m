function [ output_args ] = TrafficLightDetection()
 ...
 step_r=floor(min(size(im_edge(:,:,1)))*0.02);
 step_angle=2*pi/8;
 r_min=floor(min(size(im_edge(:,:,1)))*0.08);
 r_max=floor(min(size(im_edge(:,:,1)))*0.15);
  
  [hough_space,hough_circle,para]=do_hough_circle(im_edge(:,:,1),step_r,step_angle,r_min,r_max,0.9);
  figure;
  imshow(im);
  hold on;
...
function []=plotcircle(para)
  sita=0:pi/20:2*pi;
  x0=para(1,1);y0=para(1,2);r=para(1,3);
  %%%%%%%%%%%%%%%%%%%%%%画圆%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      for j=1:size(im,2)
       %  r=im(i,j,1);g=im(i,j,2);b=im(i,j,3);
         ...
         if (r(i,j)>200 && g(i,j)>150 && b(i,j)<150)    %is orange
           cand_region(i,j,:)=[255,97,0];
         end
         
         if (r(i,j)<150 && g(i,j)>240 && b(i,j)>220)
