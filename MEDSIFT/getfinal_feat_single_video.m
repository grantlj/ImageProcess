...
%   end;
 ...
 if (~exist([out_path,num2str(videoid),'-SIFT_FINAL_Feature.mat'],'file'))
         SIFT_raw_feat_path=[out_path,num2str(videoid),'-SIFT_RAW_Feature.mat'];
         
         %load raw feat.
         load(SIFT_raw_feat_path);
         
         %backup the original feat in feat_raw, feat_raw will save the 
         %new bow fetures.
         SIFT_raw_feat=feat_raw;
         feat_raw=[];
         
         for i=1:size(SIFT_raw_feat,2)
            now_SIFT_raw=SIFT_raw_feat{i};
            if (isempty(now_SIFT_raw))
                continue;
            end
            
            %doing bag-of-words using centers and kdtree.
            now_vec=conduct_bow(now_SIFT_raw,centers,kdtree);
       ...
       save([out_path,num2str(videoid),'-SIFT_FINAL_Feature.mat'],'feat_raw','feature_max','feature_min','feat_lag');
