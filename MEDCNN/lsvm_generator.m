...
if (~useold)
    ...
    save(['/home/liujiang/',test_object,'_CNN_train_set.mat'],'train_set','-v7.3');   %save the test_set, for the convenience of test_engine.
    cd(root);

    %save test set to file, for lsvm_test_engine use.
    save(['/home/liujiang/',test_object,'_CNN_test_set.mat'],'test_set','-v7.3');   %save the test_set, for the convenience of test_engine.
    clear test_set;  
else
    load(['/home/liujiang/',test_object,'_CNN_train_set.mat']);
end
...
for itecount=1:100   %begin iteration. maximum iteration count is 100.
   if (exist(['lsvm_model_cnn/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_',num2str(itecount),'.mat'],'file'))
       load(['lsvm_model_cnn/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_',num2str(itecount),'.mat']);
       disp('Model existed..');
   else
  ...
  if (itecount==1)
        save(['lsvm_model_cnn/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_0.mat'],'models');  %save default mean-pooling strategy for comparable test.
  end
  ...
  for object_bank_word=1:total_word
   ...
   save(['lsvm_model_cnn/',test_object,'_Word_',num2str(object_bank_word),'_1vN_Models_After_ITE_',num2str(itecount),'.mat'],'models');
