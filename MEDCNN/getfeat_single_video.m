function []=getfeat_single_video(datapath,videoid,out_path,net)
%   if (nargin<1)
%     videopath='test.avi';
%   end;
 disp(['Now handling:',num2str(videoid),'  under path;',datapath,'   output path:',out_path]); 
 if (~exist([out_path,num2str(videoid),'-CNN_Feature.mat'],'file'))
        ...
        cd(videopath);
         ...
         for i=1:size(tmp_jpgfile,2)
            if (~isempty(strfind(tmp_jpgfile{1,i},'-2.jpg')))
                ...
                feat=getfeat_single_image(tmp_im,net);
         ...
         save([out_path,num2str(videoid),'-CNN_Feature.mat'],'feature_max','feat_raw','feature_min','feature_mean','feat_lag');
