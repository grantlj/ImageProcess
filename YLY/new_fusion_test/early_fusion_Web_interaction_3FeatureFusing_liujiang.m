...
for experi_num = [1 4]
    ...
    for test_fold= 1:1:10   %每循环一次 换一组（5个）样本作为test
        mkdir('/data/collegestudent/liujiang/YLY/new_fusion_test/storage/',[char(cellstr(experi_folder(experi_num))),'+STIP+SIFT']);   %storage_path is the path that you store the results
        %---------------------------------------------------------------------------------------------------------------------
        ...
        feature_struct2 = load(['/data/collegestudent/liujiang/YLY/new_fusion_test/feature/',char(cellstr(experi_folder(19))),'/',char(cellstr(cls(1))),'/',sprintf('%06d',1)]);  %/data/collegestudent/liujiang/YLY/new_fusion_test/feature/ is the path that holds all the encoded features
        feature_struct3 = load(['/data/collegestudent/liujiang/YLY/new_fusion_test/feature/',char(cellstr(experi_folder(experi_num))),'/',char(cellstr(cls(1))),'/',sprintf('%06d',1)]);  %/data/collegestudent/liujiang/YLY/new_fusion_test/feature/ is the path that holds all the encoded features
        ...
        for cls_num = 1:1:9
            for seq_pnt = 1:1:45 %seq_pnt用来指向training_ordinal中的每一个元素
                ...
                feature_struct2 = load(['/data/collegestudent/liujiang/YLY/new_fusion_test/feature/',char(cellstr(experi_folder(19))),'/',char(cellstr(cls(cls_num))),'/',sprintf('%06d',training_ordinal(seq_pnt))]);  %/data/collegestudent/liujiang/YLY/new_fusion_test/feature/ is the path that holds all the encoded features
                feature2 = feature_struct2.encoding;
                feature_struct3 = load(['/data/collegestudent/liujiang/YLY/new_fusion_test/feature/',char(cellstr(experi_folder(experi_num))),'/',char(cellstr(cls(cls_num))),'/',sprintf('%06d',training_ordinal(seq_pnt))]);  %/data/collegestudent/liujiang/YLY/new_fusion_test/feature/ is the path that holds all the encoded features
        ...
        for cls_num = 1:1:9
            ...
            for test_seq_num = test_begin:1:test_end
                ...
                feature1 = feature_struct1.encoding;
                feature_struct2 = load(['/data/collegestudent/liujiang/YLY/new_fusion_test/feature/',char(cellstr(experi_folder(19))),'/',char(cellstr(cls(cls_num))),'/',sprintf('%06d',test_seq_num)]);  %/data/collegestudent/liujiang/YLY/new_fusion_test/feature/ is the path that holds all the encoded features
                feature2 = feature_struct2.encoding;
                feature_struct3 = load(['/data/collegestudent/liujiang/YLY/new_fusion_test/feature/',char(cellstr(experi_folder(experi_num))),'/',char(cellstr(cls(cls_num))),'/',sprintf('%06d',test_seq_num)]);  %/data/collegestudent/liujiang/YLY/new_fusion_test/feature/ is the path that holds all the encoded features
                feature3 = feature_struct3.encoding;
                feature = [feature1;feature2;feature3];
...
%         save(['storage_path/',char(cellstr(experi_folder(experi_num))),'+STIP+SIFT','/model',int2str(test_fold),'.mat'],'model'); %保存model用于单独某部分样本的识别AP检测   %storage_path is the path that you store the results
        model = [];
    end
    
    save(['/data/collegestudent/liujiang/YLY/new_fusion_test/storage/',char(cellstr(experi_folder(experi_num))),'+STIP+SIFT','/result.mat'],'result');  %storage_path is the path that you store the results
    save(['/data/collegestudent/liujiang/YLY/new_fusion_test/storage/',char(cellstr(experi_folder(experi_num))),'+STIP+SIFT','/score.mat'],'score');  %storage_path is the path that you store the results
end