...
for experi_num = 16
    ...
    for test_fold= 1:1:10   %每循环一次 换一组（5个）样本作为test
        mkdir('D:\YLY\storage\EARLY2\',[char(cellstr(experi_folder(experi_num))),'+MoSIFT']);   %D:\YLY\storage\EARLY2 is the path that you store the results
        %---------------------------------------------------------------------------------------------------------------------
        %                                                                                               load feature and label
        %---------------------------------------------------------------------------------------------------------------------
        %---------------------------training data------------------------
        
        feature_struct2 = load(['D:\YLY\',char(cellstr(experi_folder(18))),'\',char(cellstr(cls(1))),'\',sprintf('%06d',1)]);  %D:\YLY is the path that holds all the encoded features
        feature_struct3 = load(['D:\YLY\',char(cellstr(experi_folder(experi_num))),'\',char(cellstr(cls(1))),'\',sprintf('%06d',1)]);  %D:\YLY is the path that holds all the encoded features
        ...
        for cls_num = 1:1:9
            for seq_pnt = 1:1:45 %seq_pnt用来指向training_ordinal中的每一个元素
                ...
                feature_struct2 = load(['D:\YLY\',char(cellstr(experi_folder(18))),'\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',training_ordinal(seq_pnt))]);  %D:\YLY is the path that holds all the encoded features
                feature2 = feature_struct2.encoding;
                feature_struct3 = load(['D:\YLY\',char(cellstr(experi_folder(experi_num))),'\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',training_ordinal(seq_pnt))]);  %D:\YLY is the path that holds all the encoded features
        ...
        for cls_num = 1:1:9
            ...
            for test_seq_num = test_begin:1:test_end
                ...
                feature_struct2 = load(['D:\YLY\',char(cellstr(experi_folder(18))),'\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',test_seq_num)]);  %D:\YLY is the path that holds all the encoded features
                feature2 = feature_struct2.encoding;
                feature_struct3 = load(['D:\YLY\',char(cellstr(experi_folder(experi_num))),'\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',test_seq_num)]);  %D:\YLY is the path that holds all the encoded features
                feature3 = feature_struct3.encoding;
                feature = [feature2;feature3];
...
%         save(['D:\YLY\storage\EARLY2\',char(cellstr(experi_folder(experi_num))),'+MoSIFT','\model',int2str(test_fold),'.mat'],'model'); %保存model用于单独某部分样本的识别AP检测   %D:\YLY\storage\EARLY2 is the path that you store the results
        model = [];
    end
    
    save(['D:\YLY\storage\EARLY2\',char(cellstr(experi_folder(experi_num))),'+MoSIFT','\result.mat'],'result');  %D:\YLY\storage\EARLY2 is the path that you store the results
    save(['D:\YLY\storage\EARLY2\',char(cellstr(experi_folder(experi_num))),'+MoSIFT','\score.mat'],'score');  %D:\YLY\storage\EARLY2 is the path that you store the results
end