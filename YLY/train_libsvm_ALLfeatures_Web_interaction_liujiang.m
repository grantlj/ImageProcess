    ...
    for test_fold = 1:1:10   %每一次换一个test的seq
        ...
        for cls_num = 1:1:9
            for seq_pnt = 1:1:45 %seq_pnt用来指向training_ordinal中的每一个元素
                            ...
                            feature_struct = load(['D:\yly_3dsift_feature\',char(cellstr(experi_folder(experi_num))),'\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',training_ordinal(seq_pnt))]);  %D:\yly_3dsift_feature是你存放ALL文件夹的路径
                %                 feature_struct = load(['E:\tmp\',char(cellstr(cls(cls_num))),'\fisher_vec_',sprintf('%06d',training_ordinal(seq_pnt))]);  %for STIP
%                 if experi_num == 17
%                 feature_struct = load(['F:\yly\3DSIFT\fish_encoding\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',training_ordinal(seq_pnt))]); %for 3DSIFT
%                 elseif experi_num == 18
%                 feature_struct = load(['F:\yly\MoSIFT\fish_encoding\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',training_ordinal(seq_pnt))]); %for MoSIFT
%                 end
        ...
        for cls_num = 1:1:9
            for test_seq_num = test_begin:1:test_end
                           ...
                           feature_struct = load(['D:\yly_3dsift_feature\',char(cellstr(experi_folder(experi_num))),'\',char(cellstr(cls(cls_num))),'\',sprintf('%06d',test_seq_num)]);  %D:\yly_3dsift_feature是你存放ALL文件夹的路径
...
% end
    ...
    save(['D:\yly_3dsift_feature\',char(cellstr(experi_folder(experi_num))),'\b_1_c_100\result.mat'],'result');  %D:\yly_3dsift_feature是你存放ALL文件夹的路径
    save(['D:\yly_3dsift_feature\',char(cellstr(experi_folder(experi_num))),'\b_1_c_100\score.mat'],'score');  %D:\yly_3dsift_feature是你存放ALL文件夹的路径
end