...
*/
	...
	compute_descriptors( features, gauss_pyr, descr_width, descr_hist_bins, optical_flow_pyr ); // [ build descriptors of interest points ]
...
double* CMotion_SIFT_feature::motion_ori_hist( IplImage* img, int r, int c, int n, int rad, double sigma, int octav, int interval)
{
	...
	for( i = -rad; i <= rad; i++ )                                   // [ check the y area ]
		for( j = -rad; j <= rad; j++ )                               // [ check the x area ]
			if( calc_motion_mag_ori( r + i, c + j, &mag, &ori, img, 0) ) // [ gradient ]
			{
				w = exp( -( i*i + j*j ) / exp_denom );               // [ weighting ]
				bin = cvRound( n * ( ori + CV_PI ) / PI2 );          // [ oritation ]
				bin = ( bin < n )? bin : 0;                          // [ adjust oritation ]
...
@return Returns 1 if the specified pixel is a valid one and sets mag and
	ori accordingly; otherwise returns 0
*/
int CMotion_SIFT_feature::calc_motion_mag_ori( int r, int c, double* mag, double* ori, IplImage* motion_image, int flag )
{
	double dx, dy; // [ x, y derivation ]
	...
	{
		dx = cvGetReal2D( horizontal_velocity, r, c ); // [ optical flow in x ]
		dy = cvGetReal2D( vertical_velocity, r, c );   // [ optical flow in y ]
...
*/
void CMotion_SIFT_feature::compute_descriptors( CvSeq* features, IplImage*** gauss_pyr, int d, int n, IplImage*** optical_flow_pyr )
{
	struct feature_MSIFT* feat;         // [ feature ]
	struct detection_data_MSIFT* ddata; // [ data ]
	...
	for( i = 0; i < k; i++ )                                                    // [ scan though all features ]
	{
		...
		calc_motion_mag_ori(-1, -1, NULL, NULL, optical_flow_pyr[ddata->octv][ddata->intvl], 1);
		motion_hist = motion_descr_hist( ddata->r, ddata->c, feat->ori,         // [ caldulate motion histogram ]
				       ddata->scl_octv, d, n, optical_flow_pyr[ddata->octv][ddata->intvl], ddata->octv, ddata->intvl );    
		calc_motion_mag_ori(-1, -1, NULL, NULL, NULL, -1);
...
*/
double*** CMotion_SIFT_feature::motion_descr_hist( int r, int c, double ori, double scl, int d, int n, IplImage* motion_image, int octav, int interval)
{
	double*** hist; // [ histogram ]
	...
	for( i = -radius; i <= radius; i++ )                                // [ x area ]
		for( j = -radius; j <= radius; j++ )                            // [ y area ]
		{
			...
			*/
			...
			if( rbin > -1.0  &&  rbin < d  &&  cbin > -1.0  &&  cbin < d )        // [ check boundary ]
				if( calc_motion_mag_ori( r + i, c + j, &motion_mag, &motion_ori, motion_image, 0)) // [ caldulate  mag and ori ]
				{
					if( rotation ){ //Shoou-I
	...
	for( o = 0; o < octvs; o++ )          // [ scan though octaves ]
		for( i = 1; i < intvls + 3; i++ ) // [ scan though layers ]
		{
			...
			if( image_height >= 11 && image_width >= 11 ) // [ boundary check ]
			{
			    ...
			    cvCalcOpticalFlowPyrLK( prev_gray8, curr_gray8,        // [ LK method to caculate optical flow ]
