 ...
 for i=1:m
    ...
    for j=1:n
      ...
      if (~isempty(strfind(name,'.avi')))                      %traget file.
          ...
          featurename=[now_feat_set,strrep(name,'.avi','-HOGFeature.mat')];  %feture file path.
          extract_HOG_feature(videoname,featurename,15);
      end
...
function []=extract_HOG_feature(videoname,featurename,sampleFrame)
  ...
  if (exist(featurename,'file'))
      ...
      for i=1:floor(numFrames/sampleFrame)
         try
             ...
             [d]=vl_hog(frame,cellSize,'verbose');
             feat=d;
             feat_raw{i}=feat;
        catch
