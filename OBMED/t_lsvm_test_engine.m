...
function [accuracy,fval] = t_lsvm_test_engine(itecount,train_set,test_object,word_count,model_folder)
...
addpath(genpath('libsvm-3.18/'));
 ...
 train_label=train_set{datacount}.label;
 featureCount=size(trains,2);
 
 score=[];
 for i=1:featureCount
   feat0=trains{i};     %raw feat for each video.
...
%        end
 ...
 %  clc;
    [RECALL, PRECISION, INFOAP]=vl_pr(train_label,score);
   accuracy=INFOAP.ap;
