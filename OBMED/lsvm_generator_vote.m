...
%%
function [] = lsvm_generator_vote(test_object)
...
for itecount=1:100   %begin iteration. maximum iteration count is 100.
    ...
    if (itecount==1)
        save(['lsvm_model_vote/',test_object,'_1vN_Models_After_ITE_0.mat'],'model');  %save default mean-pooling strategy for comparable test.
    end
    
    %Step 2: Update theata using w,b in step 1.
    model=lsvm_update_theta(train_set,model);
    save(['lsvm_model_vote/',test_object,'_1vN_Models_After_ITE_',num2str(itecount),'.mat'],'model');
end
...
for i=1:size(train_set_global,2)                      %train_set_global{1}: pos examples. train_set_global{2}: nega examples.
    for j=1:size(train_set_global{i}.trains,2)        
        ...
        if (i==1 || i==2)
            ...
            feat_new=feat_new((present_filter_number-1)*252+1:(present_filter_number)*252);
...
%here, we use the new method, discriminatively train every SINGLE OBJECT
%BANK FILTER Pooling strategy. and fusion with vote strategies.

global present_filter_number;

ansMat=zeros(1,size(model.thetaMat(:)',1)*size(model.thetaMat(:)',2));

for i=1:177  %object bank number
   %initWeight=rand(1,size(initWeight,2));                                         %random strategy for initial weights.
   %initWeight=initWeight./sum(initWeight);
    present_filter_number=i;
    
    [tmpMat,fval, exitflag]=fmincon(@theta_opt, ...                                 %goal function.                                                    
        initWeight,...
    ...
    ansMat=ansMat+tmpMat;
end

%filter overall mean-pooling.
ansMat=ansMat./177;
